...
# purpose: Assess Linux telemetry quality from linux_events_export.json and produce linux_telemetry_quality.json.
# author: Massimo Rios
...
INPUT_JSON="linux_events_export.json"
OUTPUT_JSON="linux_telemetry_quality.json"

echo "[*] Analyzing $INPUT_JSON..."

# Calculate total events using jq
total=$(jq 'length' "$INPUT_JSON")

# Event distribution by category and source type
...
timestamp_good=$(jq '[.[] | select(.timestamp != null and .timestamp != "")] | length' "$INPUT_JSON")
hostname_good=$(jq '[.[] | select(.hostname != null and .hostname != "")] | length' "$INPUT_JSON")
source_good=$(jq '[.[] | select(.source_type != null and .source_type != "")] | length' "$INPUT_JSON")
category_good=$(jq '[.[] | select(.event_category != null and .event_category != "")] | length' "$INPUT_JSON")
...
# execve command line
exec_total=$(jq '[.[] | select(.event_category=="execve")] | length' "$INPUT_JSON")
exec_good=$(jq '[.[] | select(.event_category=="execve" and .command != null and .command != "")] | length' "$INPUT_JSON")
exec_pct=$(percentage "$exec_good" "$exec_total")

# SSH source IP and user
ssh_total=$(jq '[.[] | select(.event_category=="ssh_login_success" or .event_category=="ssh_login_failure")] | length' "$INPUT_JSON")
...
# auditd file events
file_total=$(jq '[.[] | select(.event_category=="file_access")] | length' "$INPUT_JSON")
